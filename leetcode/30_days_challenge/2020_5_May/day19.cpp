...
public:
  ...
  int next(int price) 
  {
    ...
    if(index == 0)
    {
      span_counter.push_back(1);
    }
    else
    {
      int counter = 1;
      while((index - counter) >= 0 && prices[index] >= prices[index - counter])
      {
        counter += span_counter[index - counter];
      }
      
      span_counter.push_back(counter);
      ans = counter;
    }

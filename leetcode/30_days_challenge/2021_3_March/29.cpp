...
/*
  Q: Flip Binary Tree To Match Preorder Traversal
...
{
private:
  bool doesMatch(TreeNode* node, vector<int>& voyage, int& p, vector<int>& ans)
  {
    if(node == nullptr)
    {
      return true;
    ...
    if(l != nullptr && l->val != voyage[p])
    {
      ans.push_back(node->val);
      swap(l, r);
    }
    
    return doesMatch(l, voyage, p, ans) && doesMatch(r, voyage, p, ans);
  }
...
public:
  vector<int> flipMatchVoyage(TreeNode* root, vector<int>& voyage) 
  {
    int p = 0;
    vector<int> ans;
    if(!doesMatch(root, voyage, p, ans))
    {
      ans = {-1};
    }

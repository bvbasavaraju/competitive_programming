...
public:
  int myAtoi(string s) 
  {
    ...
    int sign_found = false;
    int digits_read = false;
    int l = s.size();
    for(int i = 0; i < l; ++i)
    {
      if((s[i] < '0') || (s[i] > '9'))
      {
        if(!digits_read && !sign_found && ((s[i] == '-') || (s[i] == '+') || ((s[i] == ' ') && (ans == 0)) ) )
        {
          sign_found = ((s[i] == '-') || (s[i] == '+'));
          is_negative = (s[i] == '-');
          continue;
    ...
    ans = (is_negative == true) ? ans * -1 : ans;
    return (ans <= INT_MIN) ? INT_MIN : ((ans >= INT_MAX) ? INT_MAX : static_cast<int>(ans));
  }
};